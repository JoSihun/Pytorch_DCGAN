    ...
    for epoch in range(args.num_epochs):
        for i, images in enumerate(data_loader):
            ...
            if (i + 1) % args.log_step == 0:
                print(f'Epoch[{epoch + 1}/{args.num_epochs}], Step[{i + 1}/{total_step}], d_loss: {d_loss.data:.4f}, g_loss: {g_loss.data:.4f}')
            ...
            if (i + 1) % args.sample_step == 0:
                ...
                torchvision.utils.save_image(denorm(fake_images.data),
                                             os.path.join(args.sample_path, f'generatedimage-{epoch + 1}-{i + 1}.png'))
